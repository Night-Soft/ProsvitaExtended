...
(function (global, factory) {
  ...
  var DEFAULT_OPTIONS = {
    ...
    set rotate(value) {
      this.degrees = value;
      let endTransition = () => {
        console.log("end Transition rotate = ", value + "deg");
        zoomist.image.style.transition = "";
      }
      zoomist.image.addEventListener("transitionend", endTransition);
      zoomist.image.style.transition = "0.7s";
      let text = zoomist.image.style.transform;
      let result = text.slice(0, text.indexOf("rotate"));
      zoomist.image.style.transform = result + "rotate(" + this.degrees + "deg)";
    }
  };
  ...
  var METHODS = {
    ...
    zoom(zoomRatio, pointer) {
      ...
      setStyle(image, {
        ...newData,
        transform: `translate(${transformX}px, ${transformY}px) rotate(${options.rotate}deg)`
      });
      this.ratio = newRatio;
    ...
    move(x = 0, y = 0) {
      ...
      image.style.transform = `translate(${newTransX}px, ${newTransY}px) rotate(${options.rotate}deg)`;
      return this;
    },
  ...
  var bindEvents = (zoomist => {
    ...
    const dragMove = e => {
      ...
      image.style.transform = `translate(${transformX}px, ${transformY}px) rotate(${options.rotate}deg)`;
      zoomist.emit('drag', {
        x: transformX,
        y: transformY
      }, e);
...
`;
  ...
  class Zoomist {
    ...
    constructor(element, options = {}) {
      ...
      //this.options = Object.assign({}, DEFAULT_OPTIONS, isPlainObject(options) && options); // prev
      this.options = {}
      Object.defineProperties(this.options, Object.getOwnPropertyDescriptors(DEFAULT_OPTIONS));
